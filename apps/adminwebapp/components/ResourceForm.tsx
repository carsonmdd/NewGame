...
const openai = new OpenAI({
  apiKey: "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", dangerouslyAllowBrowser: true,
});

// Add tags in
const TAG_OPTIONS = [
  "Blue", "Red", "Green", "Cat", "Dog", "Bird",
] as const;

type TagOption = (typeof TAG_OPTIONS)[number];

export default function ResourceForm({ initialData, onSubmit, onCancel }: Props) {
  const [formData, setFormData] = useState<ResourceInput>({
    title: initialData?.title ?? "",
    description: initialData?.description ?? "",
    resourceType: (initialData?.resourceType as any) ?? "video",
    url: initialData?.url ?? "",
    tags: (initialData?.tags as string[]) ?? [],
    difficulty: (initialData?.difficulty as any) ?? "beginner",
  });
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="space-y-1">
          ...
          <select
            ...
            onChange={(e) =>
              setFormData({ ...formData, resourceType: e.target.value as any })
            }
          >
          ...
          <div className="flex flex-wrap gap-2">
            {formData.tags.map((tag) => (
              <button
                ...
                title="Remove tag"
              >
                #{tag} <span className="ml-1 text-gray-500">×</span>
              </button>
            ))}
          </div>
        )}
