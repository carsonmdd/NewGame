...
export function SavedResourcesProvider({ children }: { children: ReactNode }) {
  ...
  const saveResource = (resource: Resource) => {
    const id = resource.objectID || resource.id;

    setSavedResources((prev) => {
      const alreadySaved = prev.some((r) => (r.objectID || r.id) === id);
  ...
  const unsaveResource = (resourceId: string) => {
    setSavedResources((prev) =>
      prev.filter((r) => (r.objectID || r.id) !== resourceId)
    );
  };

  const isSaved = (resourceId: string) =>
    savedResources.some((r) => (r.objectID || r.id) === resourceId);

  const value = useMemo(
    () => ({ savedResources, saveResource, unsaveResource, isSaved }),
    [savedResources]
  );
